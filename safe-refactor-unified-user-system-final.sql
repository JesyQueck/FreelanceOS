...
-- Final version with proper UUID handling
...
-- Only update users that have corresponding client records
UPDATE public.users 
SET role = 'client' 
WHERE id::text IN (SELECT user_id::text FROM public.clients WHERE user_id IS NOT NULL)
AND role IS NULL;
...
-- STEP 4: UPDATE CONVERSATIONS TABLE FOR UNIFIED SYSTEM
...
-- Only update conversations where we can find matching users
UPDATE public.conversations 
SET client_user_id = c.user_id
FROM public.clients c
WHERE public.conversations.client_id = c.id 
AND c.user_id IS NOT NULL
AND EXISTS (SELECT 1 FROM public.users u WHERE u.id::text = c.user_id::text);

-- STEP 5: ENSURE MESSAGES TABLE HAS ALL REQUIRED COLUMNS
DO $$
BEGIN
    IF NOT EXISTS (
        SELECT 1 FROM information_schema.columns 
...
-- STEP 7: MIGRATE CLIENT PROFILES SAFELY
...
AND EXISTS (SELECT 1 FROM public.users u WHERE u.id::text = c.user_id::text)
AND NOT EXISTS (SELECT 1 FROM public.client_profiles cp WHERE cp.user_id::text = c.user_id::text);

-- STEP 8: CREATE OR UPDATE TRIGGERS FOR UPDATED_AT COLUMNS
...
-- STEP 9: UPDATED ROW LEVEL SECURITY POLICIES
