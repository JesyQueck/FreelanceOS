    ...
    <div className="space-y-6">
      ...
      {loading ? (
        ...
        <div className="grid grid-cols-1 md:grid-cols-2 lg:grid-cols-3 gap-6">
          {services.map((service) => (
            <div key={service.id} className="card p-6 flex flex-col h-full hover:scale-[1.02] transition-transform">
                ...
                <div className="flex items-center gap-1">
                  <span className={`px-2 py-1 text-xs font-medium rounded-full ${
                    service.status === 'active' 
                      ...
                      : 'bg-[var(--color-bg-secondary)]/10 text-[var(--color-text-secondary)] border border-[var(--color-border)]/20'
                  }`}>
              ...
              <div className="flex-grow">
                <p className="text-sm text-[var(--color-text-secondary)] leading-relaxed">{service.description || 'No description provided'}</p>
              </div>
              ...
              <div className="space-y-3 mt-auto pt-4">
                {service.price && (
                  <div className="flex items-center gap-2">
                    ...
                    <span className="text-sm font-medium text-[var(--color-text-primary)]">{service.price}</span>
                  </div>
                )}
                {service.timeline && (
                  <div className="flex items-center gap-2">
                    <Clock className="h-4 w-4 text-[var(--color-primary)]" />
                    <span className="text-sm text-[var(--color-text-secondary)]">{service.timeline}</span>
                  </div>
                )}
              </div>
              ...
              <div className="flex items-center gap-2 pt-4 border-t border-[var(--color-border)] mt-auto">
                ...
                <button 
                  ...
                  disabled={isSaving}
                  className="p-2 rounded-lg hover:bg-[var(--color-error)]/10 transition-colors text-[var(--color-text-secondary)] hover:text-[var(--color-error)] disabled:opacity-50 disabled:cursor-not-allowed"
      ...
      {!loading && services.length === 0 && (
        <div className="card p-12 text-center">
          ...
          <h3 className="text-lg font-semibold text-[var(--color-text-primary)] mb-2">No services yet</h3>
          <p className="text-[var(--color-text-secondary)] mb-6">Create your first service to start attracting clients</p>
          <button 
            onClick={() => setShowAddService(true)}
            className="btn btn-primary gap-2"
          >
